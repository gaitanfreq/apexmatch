...
const NAV_ITEMS = [
  ...
  { href: '/predictions', label: 'Live Predictions', icon: SignalIcon },
  { href: '/vip', label: 'Value Bets', icon: DiamondIcon, vip: true },
  { href: '/bankroll', label: 'Bankroll Tracker', icon: WalletIcon },
...
export default function Sidebar({ open, onClose }) {
  ...
  const navItems = role === 'ADMIN'
    ? [...NAV_ITEMS, { href: '/admin', label: 'Admin', icon: ShieldIcon }]
    : NAV_ITEMS;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-4 rounded-lg border border-[#232b3e] bg-[#141923]/60 p-3 text-xs">
          {status === 'loading' ? null : session ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-1.5">
                ...
                >
                  <UserIcon width={13} height={13} /> Cuenta
                </Link>
                <button
                  onClick={() => signOut({ callbackUrl: '/' })}
                  className="flex flex-1 items-center justify-center gap-1 rounded-md border border-[#232b3e] bg-black/20 py-1.5 text-[11px] font-medium text-slate-300 hover:bg-white/5"
                >
                  <LogoutIcon width={13} height={13} /> Salir
                </button>
              </div>
            </div>
          ) : (
