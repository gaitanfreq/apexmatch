  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-neon-magenta/40 bg-[#141923]/80 p-8 text-center shadow-glow-magenta backdrop-blur-lg">
      ...
      <div className="relative mx-auto max-w-sm">
        ...
        <span className="badge mb-3 bg-neon-magenta/15 text-neon-magenta glow-text-magenta">{t('match.edge')}: +7.5%</span>
        <h3 className="mb-2 text-lg font-semibold text-white">{t('paywall.title')}</h3>
        <p className="mb-6 text-sm text-ink-muted">{t('paywall.desc', { email: session.user.email })}</p>
        ...
        >
          {busy ? t('settings.redirectingStripe') : t('home.unlockVip')}
        </button>
      </div>
    </div>
  );
